...
| [superpowers/specs/2026-04-17-datasource-routing-and-pools.md](./superpowers/specs/2026-04-17-datasource-routing-and-pools.md) | **MySQL / Redis 命名池**；**`routing_key`** + **LPM**；**`prefix: "*"`** 显式默认池（与 `""` 二选一）；**策略 A**；迁移建议 **§7**；`ResolveDB` / `ResolveRedis` |
