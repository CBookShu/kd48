...
- **只读切换新库**（`routing_key` 不变）：数据同步完成后，把 **命中该域的那条规则** 的 **`pool`** 从旧池名改为新池名（新池指向新 DSN），**重启进程** 加载新配置即可；**无需**改业务代码（与 **§11**「先不热更」一致）。  
...
- [ ] 实现是否满足 **§11**（`pkg` 复用、共用 LPM、启动加载、不热更）？

---

## 11. 实现约束（已定案）

以下为实现阶段 **应遵守** 的约束（与上文语义一致；包名以实现为准）。

### 11.1 配置与路由：**先不热更**

- **路由表与各池 DSN/地址** 在进程 **启动时读取一次**；校验通过后 **建立全部池**，运行期 **`Router` 视为不可变快照**。  
- **修改路由或池配置 = 重启进程**（或未来单独做「热更」能力，**不在**当前已定案范围内）。  
- **不要求** 运行期 `routing_key → pool` 结果缓存的失效策略；若日后加缓存，须在规格中 **另段** 约定与热更的关系。

### 11.2 **pkg** 复用

- **LPM 解析、`ResolveDB` / `ResolveRedis` 语义** 放在 **公共 `pkg/`**（建议独立子目录，如 `pkg/datasource` / `pkg/dsroute`，名称实现时敲定），由 **Gateway、User、Lobby** 等复用，**禁止** 在各服务内复制一份 LPM。

### 11.3 MySQL / Redis **共用路由逻辑**

- **共用**：同一套 **规则切片** 形态（`prefix` + `pool_name`）与 **同一函数** `resolvePoolName(routing_key string) (pool_name, matched_prefix string, err error)`，内部即 **§3** LPM。`mysql_routes` 与 `redis_routes` **各自一份规则**（仍可能内容不同），但 **算法与代码路径共用**。  
- **分叉**：`ResolveDB` 在得到 `pool_name` 后查 **`map[string]*sql.DB`**（或等价句柄）；`ResolveRedis` 查 **`map[string]redis.UniversalClient`**（或项目统一类型）。  
- **启动顺序（建议）**：① 解析配置 → ② **先校验规则**（重复 `prefix`、指向不存在的池、LPM 最大档冲突等）→ **失败则进程不得进入可服务状态** → ③ **`sql.Open` / `redis.NewClient`** 建各命名池 → ④ 组装只读 `Router` 注入业务。

### 11.4 与 §9「非目标」的关系

- **动态热增池 / 路由热更**：仍为 **非目标**；§11.1 为当前 **M0 实现边界** 的正面表述。
